...
2. Read in the data set, call it "chd", and change the class of the response (chd) to a factor. You can do this using the `mutate` function from tidyverse and `as.factor`.
...
## Perform some type of variable selection procedure to determine which variables to include in the model. Hint: You will need to specify the "family" argument.
...
```{r, fig.align='center'}
heart_logistic <- heart_best_subsets_bic$BestModel 
...
```{r, eval=FALSE}
# your code here
# hint: Use predict(). Adding the type = "response" argument will automatically
# give you a probability, or you can omit that argument and convert the log odds
# to a probability by hand.
```
...
```{r, fig.align='center'}
# your code here
# hint: Use predict() with se.fit = TRUE. Then, compute the margin of error by 
# hand followed by the upper and lower bounds. That CI will be on the log
# odds scale, so lastly, convert the bounds to be probabilities. 
```
...
### Create a confusion matrix based on the best cutoff.
